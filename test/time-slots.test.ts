...
blocks.forEach((block, idx) => {
  ...
  assert(
    block.blockNumber === idx + 1,
    `Bloque ${idx + 1} tiene número correcto`,
  );
});

// Verificar que el recreo está después del bloque 2
const recreoIndex = basicSlots.findIndex(
  (s) => s.type === "break" && s.breakName === "Recreo",
);
const bloqueAntes = basicSlots
  .slice(0, recreoIndex)
...
const almuerzoIndex = basicSlots.findIndex(
  (s) => s.type === "break" && s.breakName === "Almuerzo",
);
const bloquesAntesAlmuerzo = basicSlots
  .slice(0, almuerzoIndex)
  .filter((s) => s.type === "block").length;
...
basicSlots.forEach((slot, idx) => {
  if (slot.type === "block") {
    console.log(
      `  ${idx + 1}. ${slot.time} - ${slot.endTime} | Bloque ${slot.blockNumber}`,
    );
  } else {
    console.log(
      `  ${idx + 1}. ${slot.time} - ${slot.endTime} | ${YELLOW}${slot.breakName}${RESET}`,
    );
  }
});
...
console.log(
  `  Bloques: ${middleBlocks.length}, Recreos: ${middleBreaks.length}`,
);
...
console.log(
  `  Bloques: ${noBreaksBlocks.length}, Recreos: ${noBreaksBreaks.length}`,
);
...
for (let i = 0; i < basicSlots.length; i++) {
  ...
  if (slot.time !== lastEndTime) {
    console.log(
      `${RED}✗${RESET} Gap encontrado: esperado ${lastEndTime}, encontrado ${slot.time}`,
    );
    continuityOk = false;
    testsFailed++;
    break;
