      ...
      <div className="w-full">
        {label && (
          <label className="block text-sm font-medium mb-2" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>
            {label}
            {props.required && <span style={{ color: 'var(--danger-400)', marginLeft: '0.25rem' }}>*</span>}
          </label>
        )}
        <select
          ref={ref}
        ...
        {error && (
          <p className="mt-1.5 text-sm" style={{ color: '#fca5a5', fontWeight: '500' }}>{error}</p>
        )}
        {helperText && !error && (
          <p className="mt-1.5 text-sm" style={{ color: 'rgba(255, 255, 255, 0.6)' }}>{helperText}</p>
        )}
      </div>
    );
  }
