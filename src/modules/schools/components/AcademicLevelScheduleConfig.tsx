...
export function AcademicLevelScheduleConfig({
  ...
  const tabsAndContent = (
    <>
      ...
      <div
        className={`academic-level-tabs${inline ? " academic-level-tabs-inline" : ""}`}
      >
        <button
          key="config"
      ...
      <div
        ...
        style={inline ? undefined : { maxHeight: "60vh", overflowY: "auto" }}
      >
        {activeTab === "config" ? (
          ...
          <>
            ...
            >
              ℹ️ <strong>Importante:</strong> Los recreos y almuerzos se aplican
              a toda la semana. Si necesitas horarios diferentes por día,
              configura múltiples recreos con diferentes duraciones.
            </div>
          </>
        )}
      </div>
  ...
  if (inline) {
    return <div className="schools-inline-schedule">{tabsAndContent}</div>;
  }
