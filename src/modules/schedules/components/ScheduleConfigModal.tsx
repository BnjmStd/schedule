...
export function ScheduleConfigModal({ config, onConfirm, onCancel }: ScheduleConfigModalProps) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="schedule-config-body">
            ...
            <div className="schedule-config-section">
              <h4 className="schedule-config-section-title">⏰ Jornada Escolar</h4>
              ...
              <div className="schedule-config-row">
                ...
                <div className="quick-assign-form-group">
                  ...
                  <select
                    ...
                    required
                  >
                    {TIME_OPTIONS.filter(t => t > localConfig.startTime).map(time => (
                      <option key={time} value={time}>{time}</option>
                    ))}
                  </select>
                </div>
            ...
            <div className="schedule-config-section">
              <h4 className="schedule-config-section-title">📚 Bloques de Clase</h4>
              ...
              <div className="schedule-config-row">
                <div className="quick-assign-form-group">
                  ...
                  <select
                    ...
                    onChange={(e) => setLocalConfig({ ...localConfig, blockDuration: parseInt(e.target.value) })}
                    required
                  >
                ...
                <div className="quick-assign-form-group">
                  ...
                  <select
                    ...
                    onChange={(e) => setLocalConfig({ ...localConfig, breakDuration: parseInt(e.target.value) })}
                    required
                  >
                ...
                <label>
                  <input
                    ...
                    onChange={(e) => setLocalConfig({
                      ...localConfig,
                      lunchBreak: { ...localConfig.lunchBreak, enabled: e.target.checked }
                    })}
                  />
                ...
                <div className="schedule-config-row">
                  <div className="quick-assign-form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => setLocalConfig({
                        ...localConfig,
                        lunchBreak: { ...localConfig.lunchBreak, startTime: e.target.value }
                      })}
                      required
                    >
                      {TIME_OPTIONS.filter(t => t >= localConfig.startTime && t < localConfig.endTime).map(time => (
                        <option key={time} value={time}>{time}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="quick-assign-form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => setLocalConfig({
                        ...localConfig,
                        lunchBreak: { ...localConfig.lunchBreak, endTime: e.target.value }
                      })}
                      required
                    >
                      {TIME_OPTIONS.filter(t => t > localConfig.lunchBreak.startTime && t <= localConfig.endTime).map(time => (
                        <option key={time} value={time}>{time}</option>
                      ))}
                    </select>
                  </div>
            ...
            <div className="schedule-config-preview">
              ...
              <p>
                • Jornada de {localConfig.startTime} a {localConfig.endTime}<br/>
                • Bloques de {localConfig.blockDuration} min con {localConfig.breakDuration} min de descanso<br/>
                {localConfig.lunchBreak.enabled && (
                  <>• Almuerzo: {localConfig.lunchBreak.startTime} - {localConfig.lunchBreak.endTime}</>
                )}
              </p>
            </div>
          </div>
